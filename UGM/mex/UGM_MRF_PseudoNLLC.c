...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (!mxIsClass(prhs[4],"int32")||!mxIsClass(prhs[5],"int32")||!mxIsClass(prhs[6],"int32")||!mxIsClass(prhs[7],"int32")||!mxIsClass(prhs[8],"int32")||!mxIsClass(prhs[9],"int32"))
        mexErrMsgTxt("edgeEnds, V, E, nStates, nodeMap, edgeMap, Y must be int32");
            ...
            for(s = 0; s < nStates[n]; s++) {
                ...
                if(nodeMap[n + nNodes*s] > 0) {
                    if(s == Y[i + nInstances*n]-1)
                        ...
                        obs = 0;
                    g[nodeMap[n + nNodes*s]-1] += pot[s]-obs;
                }
                ...
                for(s = 0; s < nStates[n]; s++) {
                    ...
                    if(edgeMap[s1 + maxState*(s2 + maxState*(e + nEdges*f))] > 0) {
                        if(s == Y[i + nInstances*n]-1)
                            ...
                            obs = 0;
                        g[edgeMap[s1 + maxState*(s2 + maxState*e)]-1] += pot[s]-obs;
